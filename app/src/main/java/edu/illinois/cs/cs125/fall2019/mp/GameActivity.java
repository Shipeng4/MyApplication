...
public final class GameActivity extends AppCompatActivity {
    ...
    public void updateLocation(final double latitude, final double longitude) {
        int pi = TargetVisitChecker.getTargetWithinRange(targetLats, targetLngs, path,
        ...
        if (pi != -1) {
            ...
            if (last == -1) {
                changeMarkerColor(targetLats[pi], targetLngs[pi], CAPTURED_MARKER_HUE);
                int a = TargetVisitChecker.visitTarget(path, pi);
            } else if (TargetVisitChecker.checkSnakeRule(targetLats, targetLngs, path, pi)) {
                changeMarkerColor(targetLats[pi], targetLngs[pi], CAPTURED_MARKER_HUE);
                addLine(targetLats[pi], targetLngs[pi], targetLats[path[last]], targetLngs[path[last]], PLAYER_COLOR);
                int a = TargetVisitChecker.visitTarget(path, pi);
            }
        }
